...
from Model.PPOModel_1_1.actor_critic import PPOActorCritic
from Model.PPOModel_1_1.rollout_buffer import RolloutBuffer
import random
import global_vars
...
class PPOAgent:
    def __init__(self, color, game_service, lr=0.0005, update_timestep=135, clip_param=0.1, K_epochs=6, gamma=0.99):
        ...
        self.filename = "ppoagent_112.pth"
        self.buffer_filename = "ppoagent_buffer.pt"
    ...
    def save_buffer(self):
        torch.save({
            'states': [t.cpu() for t in self.buffer.states],
            'actions': [t.cpu() for t in self.buffer.actions],
            'log_probs': [t.cpu() for t in self.buffer.log_probs],
            'state_values': [t.cpu() for t in self.buffer.state_values],
    ...
    def load_buffer(self):
        if not os.path.exists(self.buffer_filename):
            return
        data = torch.load(self.buffer_filename, map_location=device)
        self.buffer.states = [t.to(device) for t in data['states']]
        self.buffer.actions = [t.to(device) for t in data['actions']]
        self.buffer.log_probs = [t.to(device) for t in data['log_probs']]
        self.buffer.state_values = [t.to(device) for t in data['state_values']]
        self.buffer.rewards = data['rewards']
        self.buffer.is_terminals = data['is_terminals']
        self.timestep = data.get('timestep', len(self.buffer.rewards))
        print(f"✅ Rollout buffer restored ({len(self.buffer.rewards)} transitions).")
